...
mcp = FastMCP(name="Bitmart Assistant",
    instructions="""
...
async def get_crypto_market_data(crypto: str) -> dict:
    ...
    try:
        ...
        if pair_info is None:
            return {"error": f"The cryptocurrency {crypto.upper()} is not available on Bitmart"}
...
async def analyze_crypto(
    ...
    try:
        ...
        price_change_24h = ((ohlcv['close'].iloc[-1] / ohlcv['close'].iloc[-24]) - 1) * 100 if len(ohlcv) >= 24 else 0
        ...
        ema_50 = ohlcv['close'].ewm(span=50, adjust=False).mean().iloc[-1] if len(ohlcv) >= 50 else None
        
        # Support and Resistance Detection with clustering
        def find_support_resistance(prices, window=20, num_levels=5, cluster_threshold=0.005):
            """
            ...
            # Cluster the levels
            resistance_clusters = cluster_levels(potential_resistance[potential_resistance > current_price], cluster_threshold)
            support_clusters = cluster_levels(potential_support[potential_support < current_price], cluster_threshold)
        ...
        def get_sr_details(prices, window=20, cluster_threshold=0.005):
            ...
            resistance_details = cluster_levels_detailed(potential_resistance[potential_resistance > current_price], cluster_threshold)
            support_details = cluster_levels_detailed(potential_support[potential_support < current_price], cluster_threshold)
        ...
        medium_trend = detect_trend(ohlcv['close'], 50) if len(ohlcv) >= 50 else "undefined"
        long_trend = detect_trend(ohlcv['close'], 200) if len(ohlcv) >= 200 else "undefined"
        ...
        if body_size < (last_candle['high'] - last_candle['low']) * 0.1:
            ...
            candle_type = "hammer" if last_candle['close'] > last_candle['open'] else "hanging_man"
        elif upper_wick > body_size * 2 and lower_wick < body_size * 0.5:
            candle_type = "shooting_star" if last_candle['close'] < last_candle['open'] else "inverted_hammer"
        ...
        roc = ((ohlcv['close'].iloc[-1] / ohlcv['close'].iloc[-12]) - 1) * 100  # Rate of Change
        ...
        return {
            ...
            "technical_indicators": {
                ...
                "bollinger_bands": {
                    ...
                    "position": "above_upper" if current_price > bb_upper.iloc[-1] else "below_lower" if current_price < bb_lower.iloc[-1] else "within_bands"
                },
                "atr": {
                    "value": round(current_atr, 4),
                    "percentage": round(atr_percentage, 2),
                    "volatility": "high" if atr_percentage > 5 else "medium" if atr_percentage > 2 else "low"
                },
                "stochastic": {
            ...
            "support_resistance": {
                "immediate_support": round(float(support_levels[0]), 2) if len(support_levels) > 0 else None,
                ...
                "immediate_resistance": round(float(resistance_levels[0]), 2) if len(resistance_levels) > 0 else None,
            ...
            "trading_recommendation": {
                "bias": market_bias,
                "confidence": "high" if abs(signal_strength) >= 3 else "medium" if abs(signal_strength) >= 1 else "low",
                "key_levels": {
                    "stop_loss_long": round(support_levels[0] * 0.99, 2) if len(support_levels) > 0 else None,
                    "take_profit_long": round(resistance_levels[0] * 1.005, 2) if len(resistance_levels) > 0 else None,
                    "stop_loss_short": round(resistance_levels[0] * 1.01, 2) if len(resistance_levels) > 0 else None,
                    "take_profit_short": round(support_levels[0] * 0.995, 2) if len(support_levels) > 0 else None
                }
            }
        }
    ...
    try:
        ...
        for position in positions:
            try:
                ...
                data_points_needed = max(historical_days * 24, 168 * 2)  # At least 2 weeks for weekly returns
                ohlcv = await client.get_last_ohlcv(
                    position.pair,
                    "1h",
                    limit=data_points_needed
                )
                ...
                for horizon_name, hours in time_horizons.items():
                    if horizon_name == "1w":
                        ...
                        returns = (ohlcv['close'] / ohlcv['close'].shift(hours) - 1).dropna()
                        ...
                        if portfolio_returns_by_horizon[horizon_name] is None:
                            portfolio_returns_by_horizon[horizon_name] = weighted_returns
                        else:
                            # Align indices before adding
                            portfolio_returns_by_horizon[horizon_name] = portfolio_returns_by_horizon[horizon_name].add(weighted_returns, fill_value=0)
                ...
                if position.liquidation_price > 0:
                    if position.side == "long":
                        liq_distance = ((position.current_price - position.liquidation_price) / position.current_price) * 100
                    else:
                        liq_distance = ((position.liquidation_price - position.current_price) / position.current_price) * 100
                else:
                ...
                position_risk = {
                    ...
                    "risk_score": "HIGH" if liq_distance < 10 or position_risk_pct > 30 else "MEDIUM" if liq_distance < 20 or position_risk_pct > 15 else "LOW"
                }
        ...
        if len(positions) > 1:
            ...
            for position in positions:
                try:
                    ohlcv = await client.get_last_ohlcv(position.pair, timeframe, limit=100)
        ...
        risk_summary = {
            ...
            "exposure_pct_of_balance": round((total_position_value / balance.total) * 100, 2),
            "total_unrealized_pnl": round(total_unrealized_pnl, 2),
            "avg_leverage": round(sum(p.leverage for p in positions) / len(positions), 2),
            "high_risk_positions": sum(1 for p in position_risks if isinstance(p, dict) and p.get("risk_score") == "HIGH"),
            "diversification": "LOW" if len(positions) == 1 else "MEDIUM" if len(positions) <= 3 else "HIGH",
            "recommendation": get_risk_recommendation(position_risks, portfolio_var, balance.total)
        }
        ...
        return {
            ...
            "correlation_matrix": correlation_matrix if correlation_matrix else "N/A - Single position or data unavailable"
        }
        
    except Exception as e:
        return {"error": f"Error calculating position risk: {str(e)}"}

def get_risk_recommendation(position_risks: List[Dict], portfolio_var: Dict, total_balance: float) -> str:
    ...
    if portfolio_var and "1d" in portfolio_var:
        ...
        if var_1d_amount > total_balance * 0.2:
            recommendations.append("Critical: Potential 1-day loss exceeds 20% of balance")
        elif var_1d_amount > total_balance * 0.1:
            recommendations.append("Warning: Potential 1-day loss exceeds 10% of balance")
    ...
    high_risk_count = sum(1 for p in position_risks if isinstance(p, dict) and p.get("risk_score") == "HIGH")
    if high_risk_count > 0:
        recommendations.append(f"{high_risk_count} position(s) at high risk - consider reducing size or adding stops")
    
    # Check concentration
    max_weight = max((p.get("position_weight", 0) for p in position_risks if isinstance(p, dict)), default=0)
    if max_weight > 30:
        recommendations.append("Position concentration too high - consider diversifying")
